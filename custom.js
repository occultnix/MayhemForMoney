...
function timeAdjust(crimeLoc, crimeTime, doMove) {
	...
	captureChance = 0 															// Reset the chance of capture
	chipEncounter = false;
	ramonEncounter = false;
	viperEncounter = false;
...
// Missing the entire moral check system, but we never got to determining moral standards on a per-crime basis
function checkEnding(bounty) {
	if (bounty < 1000) {						// If the player got less than 1K, bad ending
		console.log('bad');
		return 'bad';
	} else if (bounty > 500000) {		// If the player got more than 500K, good ending
		console.log('good');
		return 'good';
	} else {												// Anything in-between gets the meh ending
		console.log('meh');
		return 'meh';
	}
}
